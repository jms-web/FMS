...
#Region "検索"
    Private Function FunSRCH() As DataTable
        ...
        '---職番検索
        If Me.CmbBUMON_KB.SelectedValue <> "" Then
            If sbSQLWHERE.Length = 0 Then
                sbSQLWHERE.Append(" WHERE SYOKUBAN = '" & Me.CmbBUMON_KB.SelectedValue & "' ")
            Else
                sbSQLWHERE.Append(" AND SYOKUBAN =  '" & Me.CmbBUMON_KB.SelectedValue & "' ")

            End If
        Else
            If CmbBUMON_KB.Text.ToString.IsNullOrWhiteSpace = False Then

                If sbSQLWHERE.Length = 0 Then
                    sbSQLWHERE.Append(" WHERE SYOKUBAN  LIKE '%" & Me.CmbBUMON_KB.Text.Trim & "%' ")
                Else
                    sbSQLWHERE.Append(" AND SYOKUBAN  LIKE '%" & Me.CmbBUMON_KB.Text.Trim & "%' ")
...
#Region "コントロールイベント"
    ...
    Private Sub tlpFilter_Paint(sender As Object, e As PaintEventArgs) Handles tlpFilter.Paint
