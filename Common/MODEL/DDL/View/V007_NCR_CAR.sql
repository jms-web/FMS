...
SELECT 
	...
	,dbo.SC02_GET_TAIRYU_NISSU('1',V002.HASSEI_YMD,CONVERT(nvarchar,GETDATE(),112)) TOTAL_TAIRYU_NISSU --累計滞留日数
	,CASE CLOSE_FG 
	 WHEN '0' THEN
	...
	,(CASE V005.CLOSE_FG 
	 WHEN '0' THEN
		(CASE WHEN V005.SYOCHI_YOTEI_YMD <> '' 
			-- 処置予定日はST8以降から反映し、それ以前は処置日時から産出する
			AND isnull((SELECT MAX(SYONIN_JUN) FROM D004_SYONIN_J_KANRI WHERE SYONIN_HOKOKUSYO_ID = 2 AND HOKOKU_NO = V005.HOKOKU_NO AND SYONIN_HANTEI_KB = '0' ),999) >= 80 
	...
	ELSE '0'
	END)		AS TAIRYU_NISSU									--滞留日数								
	,dbo.SC02_GET_TAIRYU_NISSU('1',V005.FUTEKIGO_HASSEI_YMD,CONVERT(nvarchar,GETDATE(),112)) TOTAL_TAIRYU_NISSU	--累計滞留日数
	--,CASE WHEN dbo.SC02_GET_TAIRYU_NISSU('1'
	--									,(SELECT MAX(UPD_YMDHNS) FROM D004_SYONIN_J_KANRI
	--									  WHERE SYONIN_HOKOKUSYO_ID = 2 
	--									  AND HOKOKU_NO = V005.HOKOKU_NO
	...
	-- ELSE '0' END AS TAIRYU_FG		--滞留フラグ
	,CASE WHEN dbo.SC02_GET_TAIRYU_NISSU('1'
										,CASE WHEN ISNULL(RTRIM(V005.SYOCHI_YOTEI_YMD),'')=''
