...
/****** Object:  View [dbo].[V007_NCR_CAR]    Script Date: 2018/09/17 16:51:40 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	,isnull((SELECT MAX(SYONIN_JUN) FROM D004_SYONIN_J_KANRI 
			 ...
			 AND SYONIN_YMDHNS = ' ' ),0) AS SYONIN_JUN		--承認順
	,isnull((SELECT SYONIN_NAIYO FROM M014_SYONIN_ROUT
	  ...
	  AND SYONIN_JUN = isnull((SELECT MAX(SYONIN_JUN) FROM D004_SYONIN_J_KANRI 
							   WHERE SYONIN_HOKOKUSYO_ID = 1 
							   AND HOKOKU_NO = V002.HOKOKU_NO 
							   AND SYONIN_YMDHNS = ' '),0)),(SELECT SYONIN_NAIYO fROM M014_SYONIN_ROUT WHERE SYONIN_HOKOKUSYO_ID= 1 AND SYONIN_JUN =999 )) AS SYONIN_NAIYO	--承認内容（ステージ）
	...
	,isnull((SELECT SYAIN_ID FROM D004_SYONIN_J_KANRI 
	  ...
	  AND SYONIN_JUN = (SELECT MIN(SYONIN_JUN) FROM D004_SYONIN_J_KANRI 
						WHERE SYONIN_HOKOKUSYO_ID = 1 
						  AND HOKOKU_NO = V002.HOKOKU_NO 
						  AND SYONIN_YMDHNS = '')),'') AS GEN_TANTO_ID			--承認担当者ID（現処置担当者ID）
	  ...
	  SYAIN_ID = (SELECT SYAIN_ID FROM D004_SYONIN_J_KANRI 
				 WHERE SYONIN_HOKOKUSYO_ID = 1 
				  ...
				  AND SYONIN_JUN = (SELECT MIN(SYONIN_JUN) FROM D004_SYONIN_J_KANRI 
									WHERE SYONIN_HOKOKUSYO_ID = 1 
									AND HOKOKU_NO = V002.HOKOKU_NO 
									AND SYONIN_YMDHNS = ''))),'')	AS GEN_TANTO_NAME	--承認担当者名
	,(SELECT MAX(SYONIN_YMDHNS) FROM D004_SYONIN_J_KANRI
	  WHERE SYONIN_HOKOKUSYO_ID = 1 
	   AND HOKOKU_NO = V002.HOKOKU_NO) AS SYONIN_YMDHNS							--承認日時（処置実施日および前処置実施日）
	,CASE CLOSE_FG 
	 WHEN '0' THEN
	...
	,isnull((SELECT SYAIN_ID FROM D004_SYONIN_J_KANRI 
	  ...
	  AND SYONIN_JUN = (SELECT MIN(SYONIN_JUN) FROM D004_SYONIN_J_KANRI 
						WHERE SYONIN_HOKOKUSYO_ID = 2 
						  AND HOKOKU_NO = V005.HOKOKU_NO 
						  AND SYONIN_YMDHNS = '')),0) AS GEN_TANTO_ID			--承認担当者ID（現処置担当者ID）
	,isnull((SELECT SIMEI FROM M004_SYAIN
	  WHERE 
	  SYAIN_ID = (SELECT SYAIN_ID FROM D004_SYONIN_J_KANRI 
				 WHERE SYONIN_HOKOKUSYO_ID = 2 
	...
	,(SELECT MAX(SYONIN_YMDHNS) FROM D004_SYONIN_J_KANRI
	  WHERE SYONIN_HOKOKUSYO_ID = 2 
	   AND HOKOKU_NO = V005.HOKOKU_NO) AS SYONIN_YMDHNS							--承認日時（処置実施日および前処置実施日）
	,dbo.SC02_GET_TAIRYU_NISSU('1'
						   ,(SELECT MAX(SYONIN_YMDHNS) FROM D004_SYONIN_J_KANRI
							 WHERE SYONIN_HOKOKUSYO_ID = 2 
							  AND HOKOKU_NO = V005.HOKOKU_NO)
