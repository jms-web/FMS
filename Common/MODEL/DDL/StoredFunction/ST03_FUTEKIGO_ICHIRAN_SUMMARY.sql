...
ALTER PROCEDURE [dbo].[ST03_FUTEKIGO_ICHIRAN_SUMMARY] @BUMON_KB VARCHAR(2)=NULL --部門区分
	...
	,@GENIN_BUNSEKI_KB1 NVARCHAR(500) = NULL --原因分析区分1でWHERE条件の内容をそのままセットする
	,@GENIN_BUNSEKI_KB2 NVARCHAR(500) = NULL --原因分析区分2でWHERE条件の内容をそのままセットする
...
BEGIN TRY
	...
	SET @G_SQL  = N'';
	SET @G_SQL += N' SYONIN_HOKOKUSYO_ID';-- 4.承認報告書ID	
	SET @G_SQL += N',SYONIN_HOKOKUSYO_R_NAME';-- 6.承認報告書略名
	...
	BEGIN
		...
		SET @G_SQL += N',FUTEKIGO_JYOTAI_KB';--23.不適合状態区分
		SET @G_SQL += N',FUTEKIGO_JYOTAI_NAME';--24.不適合状態区分
	END
	...
	BEGIN
		...
		SET @G_SQL += N',JIZEN_SINSA_HANTEI_KB';--25.事前審査判定区分
		SET @G_SQL += N',JIZEN_SINSA_HANTEI_NAME';--26.事前審査判定区分名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',ZESEI_SYOCHI_YOHI_KB';--27.是正処置要否区分
		SET @G_SQL += N',ZESEI_SYOCHI_YOHI_NAME';--28.是正処置要否区分名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',SAISIN_IINKAI_HANTEI_KB';--29.再審委員会判定区分
		SET @G_SQL += N',SAISIN_IINKAI_HANTEI_NAME';--30.再審委員会判定区分名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KENSA_KEKKA_KB';--31.検査結果区分
		SET @G_SQL += N',KENSA_KEKKA_NAME';--32.検査結果区分名
	END

	IF @KONPON_YOIN_KB1 IS NOT NULL --RTRIM(LTRIM(@KONPON_YOIN_KB1)) <> ''
	BEGIN
		SET @S_SQL += N',KONPON_YOIN_KB1';--33.根本要因区分1
		SET @S_SQL += N',KONPON_YOIN_NAME1';--34.根本要因区分名1

		SET @G_SQL += N',KONPON_YOIN_KB1';--33.根本要因区分1
		SET @G_SQL += N',KONPON_YOIN_NAME1';--34.根本要因区分名1
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KONPON_YOIN_KB2';--33.根本要因区分2
		SET @G_SQL += N',KONPON_YOIN_NAME2';--34.根本要因区分名2
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KISEKI_KOTEI_KB';--37.帰責工程区分
		SET @G_SQL += N',KISEKI_KOTEI_NAME';--38.帰責工程区分名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KISO_TANTO_ID';--40.起草担当者ID
		SET @G_SQL += N',KISO_TANTO_NAME';--41.起草担当者名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',BUMON_KB';--47.部門区分
		SET @G_SQL += N',BUMON_NAME';--48.部門区分
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KOKYAKU_HANTEI_SIJI_KB';--49.顧客判定指示区分
		SET @G_SQL += N',KOKYAKU_HANTEI_SIJI_NAME';--50.顧客判定指示区分名
	END
	...
	BEGIN
		...
		SET @G_SQL += N',KOKYAKU_SAISYU_HANTEI_KB';--51.顧客最終判定区分
		SET @G_SQL += N',KOKYAKU_SAISYU_HANTEI_NAME';--52.顧客最終判定区分名
	END
