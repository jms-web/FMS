                ...
                Case "担当"
                    ...
                    With dsList.Tables(0)
                        For intCNT = 0 To .Rows.Count - 1
                            Dim Trow As DataRow = dt.NewRow()
                            '
                            Trow("DISP") = $"{ .Rows(intCNT).Item("SIMEI")}({ .Rows(intCNT).Item("SYAIN_NO")})"
                            Trow("VALUE") = .Rows(intCNT).Item("SYAIN_ID")
                            Trow("DEL_FLG") = CBool(.Rows(intCNT).Item("DEL_FLG"))
...
#Region "               社内CD実績"
                ...
                Case "社内CD実績"
                    ...
                    With dsList.Tables(0)
                        For intCNT = 0 To .Rows.Count - 1
                            Dim Trow As DataRow = dt.NewRow()
                            If Not .Rows(intCNT).Item("SYANAI_CD").ToString.IsNulOrWS Then
                                Trow("DISP") = .Rows(intCNT).Item("SYANAI_CD")
                                Trow("VALUE") = .Rows(intCNT).Item("SYANAI_CD")
                                'Trow("DEL_FLG") = CBool(.Rows(intCNT).Item("DEL_FLG"))
                                Trow("BUMON_KB") = .Rows(intCNT).Item("BUMON_KB")
...
#Region "               部品番号実績"
                ...
                Case "部品番号実績"
                    ...
                    sbSQL.Append("SELECT DISTINCT")
                    sbSQL.Append("  V007.BUHIN_BANGO")
                    sbSQL.Append(" ,ISNULL(V106.BUMON_KB,'') AS BUMON_KB")
                    sbSQL.Append(" ,ISNULL(V106.BUHIN_NAME,'') AS BUHIN_NAME")
                    sbSQL.Append(" ,ISNULL(V106.SYANAI_CD,'') AS SYANAI_CD")
                    sbSQL.Append(" ,ISNULL(V106.KISYU_ID,0) AS KISYU_ID")
                    sbSQL.Append(" ,ISNULL(V106.TOKUI_ID,0) AS TOKUI_ID")
                    sbSQL.Append(" FROM V007_NCR_CAR V007")
                    sbSQL.Append(" LEFT JOIN VWM107_BUHIN_KISYU V106")
                    ...
                    With dsList.Tables(0)
                        For intCNT = 0 To .Rows.Count - 1
                            If dt.Rows.Contains(.Rows(intCNT).Item("ITEM_NAME")) = False Then
                                ...
                                Trow("DEL_FLG") = IIf(.Rows(intCNT).Item("DEL_YMDHNS").ToString.IsNulOrWS, False, True)
                                dt.Rows.Add(Trow)
                            End If
                        Next intCNT
                    End With
